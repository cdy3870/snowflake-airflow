...
def query_data():
	...
	FORECAST_DB.FORECAST_SCHEMA.TIDES_TB t2 ON t1.TIMESTAMP_TIMESTAMP = t2.TIMESTAMP_TIMESTAMP
	'''
...
def set_up():
	...
	sql = "CREATE TABLE IF NOT EXISTS FORECASTS_TB_TEMP(id VARCHAR(50), timestamp_timestamp NUMERIC, timestamp_dt DATETIME, weather_temperature NUMERIC, \
								weather_condition VARCHAR(50), wind_speed FLOAT(), wind_direction FLOAT(), surf_min FLOAT(), \
								surf_max FLOAT())"
	cur.execute(sql)

	sql = "CREATE TABLE IF NOT EXISTS FORECASTS_TB(id VARCHAR(50), timestamp_timestamp NUMERIC, timestamp_dt DATETIME, weather_temperature NUMERIC, \
								weather_condition VARCHAR(50), wind_speed FLOAT(), wind_direction FLOAT(), surf_min FLOAT(), \
								surf_max FLOAT())"
	cur.execute(sql)

	sql = "CREATE TABLE IF NOT EXISTS TIDES_TB_TEMP(id VARCHAR(50), timestamp_timestamp NUMERIC, timestamp_dt DATETIME, type VARCHAR(20), HEIGHT FLOAT())"
	cur.execute(sql)

	sql = "CREATE TABLE IF NOT EXISTS TIDES_TB(id VARCHAR(50), timestamp_timestamp NUMERIC, timestamp_dt DATETIME, type VARCHAR(20), HEIGHT FLOAT())"
	cur.execute(sql)
...
def update_with_df(file, cols, conn, table_name):
	...
	SELECT * FROM {table_name}_TB_TEMP EXCEPT SELECT * FROM {table_name}_TB"
...
def update_db():
	...
	cols = ['id', 'timestamp_timestamp', 'timestamp_dt', 'weather_temperature', 'weather_condition', 'wind_speed', 'wind_direction', 'surf_min', 'surf_max']
	file = "forecasts_data.csv"
	sql = update_with_df(file, cols, conn, table_name)
	cur.execute(sql)

	table_name = "TIDES"
	cols = ['id', 'timestamp_timestamp', 'timestamp_dt', 'type', 'height']
	file = "tides_data.csv"
	sql = update_with_df(file, cols, conn, table_name)
	cur.execute(sql)
...
def test():
	...
	query = '''
	SELECT * FROM FORECAST_DB.FORECAST_SCHEMA.FORECASTS_TB_TEMP
	'''
